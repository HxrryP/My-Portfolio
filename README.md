...
2. Upload the **contents** of this folder to the publishing root, replacing the old `index.html` and related site files. Keep the `assets` folder and `harry-portrait.jpg` inside it, and upload both `script.js` and `projects.js`. GitHub Pages must be set to publish from the same branch and folder as before (typically `main` and `/(root)`).
